...
%% @doc Redirect to the authorize uri of Google
...
-module(controller_google_authorize).
-author("Marc Worrell <marc@worrell.nl>").
...
redirect_location(Context) ->
    ...
    z_context:set_session(google_state, State, Context),
    {AppId, _AppSecret, Scope} = mod_google:get_config(Context),
    ContextNoLang = z_context:set_language(undefined, Context),
    RedirectUrl = z_convert:to_list(
                        z_context:abs_url(
                            z_dispatcher:url_for(google_redirect, ContextNoLang),
                            Context)),
